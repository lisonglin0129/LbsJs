/**
  *   ===================================================================
  *   文件名：lbs.min.js
  *   版权：Copyright 2019-2050 TL Tech. Co. Ltd.  All Rights Reserved.
  *   描述： Js模板引擎
  *   ===================================================================
  *   修改内容：新加
  *   修改人：李
    ...
    return lbs = {
        assign: function(id, datas, tps, hex) {
            ...
            if (tps == false) {
                ...
                templateContext = templateContext.replace(/\'/g, "\"").trim().replace(/(\n)+|(\r\n)+/g, "").replace(/'/g,'"');;
                ...
                return this.loader_template(id, function(e) {
                    ...
                    if (hexs != false) {
                        if (lbs.string.eqlower(hexs, "hex")) {
                            tmp = lbs.string.hextoString(e)
                        }
                        if (lbs.string.eqlower(hexs, "base64")) {
                            tmp = lbs.string.base64decode(e)
                        }
                        if (lbs.string.eqlower(hexs, "hash")) {
                            tmp = lbs.string.hextoString(e);
                            tmp = lbs.string.base64decode(tmp)
                        }
                    }
                    tmp = tmp.replace(/(\n)+|(\r\n)+/g, "").replace(/'/g,'"');
                    templateCodeArrays = _this.depin_check_language(tmp);
        ...
        d:function(options){
			 ...
			 let debug  = false;
			 let param  = arguments;
			 let logo = "https://thirdqq.qlogo.cn/g?b=sdk&k=Y9ptVzLofzGKAFB0OnvoFQ&s=100&t=1592403748";
						 ...
						 if(debug){
							 if(param.length == 0){
								...
								for(var j = 0; j<param.length;j++){
									if(param[j] !="logo_consolelog"){
									  if(typeof(param[j])=="object"){
										  console.log("%c ["+t+" DEBUG ]  %O","font-size:1rem; color:"+lbs.get.getRandomColor()+"; ",param[j]);
								   	  }else{
										  console.log("%c ["+t+" DEBUG ]  "+param[j],"font-size:1rem; color:"+lbs.get.getRandomColor()+"; ");
								   	  }
									}
								}
							 }
        ...
        get: {
            Id: function(obj, fun) {
                if (undefined == fun) {
                    if(lbs.is.object(obj) ){
                       return obj;
                    }
                    if(obj.substr(0,1) == "."){
                        return lbs.get.className(obj);
                    }
        ...
        set:{
            ...
            SelectText:function(obj, style){
                var dm = lbs.add.Dom(document.head,"style");
                var StyleStr = "";
                for (x in style){
                    StyleStr = StyleStr + x + ":" + style[x]+";";
                }
                StyleStr = "::selection {" + StyleStr + "}";
                console.log(StyleStr);
                document.body.addEventListener("mousedown",function(){
                    dm.innerHTML=StyleStr;
                });
            }
        },
